...
#define NUM_PIXELS 4
#define PWM 18 // the built in LED on the Pico
    ...
    while(1) {
        for(int i = 0; i < 360; i++) {
            ...
            for(int j = 0; j < NUM_PIXELS; j++) {
                wsColor color = HSBtoRGB((i + j * 90) % 360, 1, .10);
...
void intialize_PWM() {
    ...
    pwm_set_gpio_level(PWM, wrap / 2); // set the duty cycle to 50%

}

// servo angle
void servo_angle(float angle) {
    pwm_set_gpio_level(PWM, 1250 + 5000*angle/180);
}
